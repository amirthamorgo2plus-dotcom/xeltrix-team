...
// Map auth user id -> last_sign_in_at (paginated; small user base).
async function lastSignInByUser(sb: SupabaseClient): Promise<Map<string, string | null>> {
...
export default async function OrgsAdminPage() {
  ...
  const [{ data: teams }, { data: members }, lastSignIn, storageByUser] = await Promise.all([
  ...
  (members ?? []).forEach((m) => {
    ...
    const ts = lastSignIn.get(m.user_id);
    if (ts) {

...
export default async function OpportunitiesPage() {
  ...
  const [{ data: opps }, { data: leads }, settings] = await Promise.all([
    supabase
      .from("opportunities")
      .select("id, title, value, stage, close_date, lead:leads(id, name)")
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3 xl:grid-cols-6">
        {STAGES.map((stage) => {
          ...
          const sum = list.reduce((acc, o) => acc + Number(o.value || 0), 0);
          return (
            <Card key={stage} className="flex flex-col">
              <CardHeader>
                ...
                <div className="text-xs text-zinc-500">{fmtMoney(sum, currency)}</div>
              </CardHeader>
              <CardContent className="flex flex-1 flex-col gap-2">
                {list.length === 0 ? (
                  ...
                  list.map((o) => {
                    const leadName =
                      Array.isArray(o.lead) ? o.lead[0]?.name : (o.lead as { name?: string } | null)?.name;
                    return (
                      <div
                        key={o.id}
                      ...
                      >
                        ...
                        <div className="mt-1 flex items-center justify-between">
                          <span className="text-xs font-medium">
                            {fmtMoney(Number(o.value), currency)}
                          </span>

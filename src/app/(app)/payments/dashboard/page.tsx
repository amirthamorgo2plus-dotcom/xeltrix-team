...
export default async function PaymentsDashboardPage({
  ...
  for (let m = 1; m <= 12; m++) {
    monthIsos.push(`${year}-${String(m).padStart(2, "0")}-01`);
  }

  const monthRows = monthIsos.map((mi) => {
    const monthBudget = items.reduce((s, it) => s + expectedBudgetForMonth(it, mi), 0);
  ...
  const totalSpent = (payments ?? []).reduce((s, p) => s + Number(p.actual ?? 0), 0);
  const remaining = annualBudget - totalSpent;
  const pctOfBudget = annualBudget > 0 ? Math.round((totalSpent / annualBudget) * 100) : 0;
  ...
  (payments ?? []).forEach((p) => {
    itemSpend.set(p.item_id, (itemSpend.get(p.item_id) ?? 0) + Number(p.actual ?? 0));
  });
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <form action="/payments/dashboard" className="flex items-end gap-2">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KpiCard label="Annual budget" value={fmtMoney(annualBudget)} hint="Expected full year" />
        <KpiCard
          label="Spent"
        ...
        />
        <KpiCard label="Payments made" value={paymentCount} hint="Across the year" />
      </div>

      {items.length === 0 ? (
        <EmptyState title="No items defined" hint="Add items in the Monthly tab first." />
      ) : (
              ...
              {monthRows.map((r) => {
                ...
                return (
                  <tr key={r.month} className="border-t border-zinc-200 dark:border-zinc-800">
                    ...
                    <td className="py-2 pr-4 text-right tabular-nums">{fmtMoney(r.monthBudget)}</td>
                    <td className="py-2 pr-4 text-right tabular-nums">{fmtMoney(r.monthSpent)}</td>
                    <td
                ...
                .map(([cat, v]) => {
                  ...
                  return (
                    <tr key={cat} className="border-t border-zinc-200 dark:border-zinc-800">
                      ...
                      <td className="py-2 pr-4 text-right tabular-nums">{fmtMoney(v.budget)}</td>
                      <td className="py-2 pr-4 text-right tabular-nums">{fmtMoney(v.spent)}</td>
                      <td
                        className={`py-2 pr-4 text-right tabular-nums ${
                          rem >= 0
                            ? "text-emerald-600 dark:text-emerald-400"
              ...
              <tbody>
                {topItems.map((it) => (
                  <tr key={it.id} className="border-t border-zinc-200 dark:border-zinc-800">
                    ...
                    <td className="py-2 pr-4 text-right tabular-nums">{fmtMoney(it.spent)}</td>
                  </tr>
                ))}
              </tbody>
            </table>

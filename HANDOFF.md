...
⚠️ **As of Jun 2026 this team's Zoho contacts have EMPTY addresses** (billing/shipping objects exist but all fields are blank; `addresses:[]`). So customers-on-map shows nothing until addresses are entered in Zoho. The pipeline is correct and will populate automatically once Zoho contacts have addresses. The sync message reports "N of M contacts had an address in Zoho". (Zoho's address object also has its own lat/lng fields — if those get filled, we could use them directly and skip geocoding.) Meanwhile, customers added on-site via the visit check-in flow DO get GPS coordinates and appear on the map.
